    ...
    public class Game1 : Game
    {
        ...
        protected override void LoadContent()
        {
            ...
            burger = new Burger(Content, @"graphics\burger", GameConstants.WindowWidth / 2,
                (int)(GameConstants.WindowHeight * 0.875), burgerShot);
            ...
            healthString = JoinStringAndInt(GameConstants.HealthPrefix,burger.Health);
            scoreString = JoinStringAndInt(GameConstants.ScorePrefix, score);
        }
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            for (int i = 0; i < bears.Count; i++)
            {
                for (int j = i + 1; j < bears.Count; j++)
                {
                    ...
                    {
                        CollisionResolutionInfo currentCollisionResolutionInfo =
                            ...
                            bears[i].CollisionRectangle, bears[j].Velocity, bears[j].CollisionRectangle);
                        if (currentCollisionResolutionInfo != null)
                        {
                            if (currentCollisionResolutionInfo.FirstOutOfBounds)
                            {
                                bears[i].Active = false;
                            }
                            else
                            {
                                bears[i].Velocity = currentCollisionResolutionInfo.FirstVelocity;
                                bears[i].DrawRectangle = currentCollisionResolutionInfo.FirstDrawRectangle;
                                teddyBounce.Play();
                            }
                            if (currentCollisionResolutionInfo.SecondOutOfBounds)
                            {
                                bears[j].Active = false;
                            }
                            else
                            {
                                bears[j].Velocity = currentCollisionResolutionInfo.SecondVelocity;
                                bears[j].DrawRectangle = currentCollisionResolutionInfo.SecondDrawRectangle;
                                teddyBounce.Play();
                            }
                        }
                    }
            ...
            foreach(TeddyBear bear in bears)
            {
                if(bear.Active && bear.CollisionRectangle.Intersects(burger.CollisionRectangle) && !burgerDead)
                {
                    burger.Health -= GameConstants.BearDamage;
                    healthString = JoinStringAndInt(GameConstants.HealthPrefix, burger.Health);
                    bear.Active = false;
                    CheckBurgerKill();
                    explosions.Add(new Explosion(explosionSpriteStrip, bear.Location.X, bear.Location.Y,this.explosion));
                    burgerDamage.Play();
                }
            }
            ...
            foreach(Projectile projectile in projectiles)
            {
                if (projectile.Type == ProjectileType.TeddyBear &&
                    projectile.CollisionRectangle.Intersects(burger.CollisionRectangle) && !burgerDead)
                {
        ...
        public static Texture2D GetProjectileSprite(ProjectileType type)
        {
            ...
            // I decided to use a switch / case / default solution , maybe in the future it will amke more sense
            Texture2D returnSprite;
            switch(type)
            {
        ...
        private string JoinStringAndInt(string prefix, int value)
        {
